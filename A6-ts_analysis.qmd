...
```{r}
# Exclude plots with more than 22 NA in training period or more than 50 NAs in total
l8_ts_plt <- l8_ts_plt %>% 
  group_by(plotID) %>% 
...
x <- decomposition_model(l8_armax_plt[1,]$`ARIMA(ndvi_int ~ prcp + prcp_lag1 + prcp_lag2 + pdq(d = 0) + 
    PDQ(D = 0), stepwise = T, ic = "aicc")`)
```
...
l8_fc_plt %>% 
  ...
  labs(y='NDVI', x='Time', title=paste('Actual (black) vs predicted (blue) vegetation including the 50 and 80% CI'))
```
