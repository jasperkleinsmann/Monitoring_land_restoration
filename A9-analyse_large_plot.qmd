...
Visualise
```{r}
mapview(plot_areas, zcol='mae')
mapview(plot_areas[plot_areas$plotID==plots_large_id[4], 'mae'], zcol='mae', legend=T)

st_write(plot_areas, dsn='output/plot_data/other/plots_areas_green.geojson', driver='GeoJSON')

plots_dt[plots_dt$plotID==plots_large_id[4],]$mae
plt_stat_fit %>% filter(plotID==plots_large_id[1])
...
fc_plot %>% 
  ...
  ggplot()+
  #geom_point(data = ts_plot[ts_plot$areaID==plt,], aes(x = date, y = ndvi), alpha=0.2,size=2)+
  #geom_vline(xintercept = plots[plots$plotID==plt,]$plant_date, lty=3, alpha=1)+
  #geom_ribbon(aes(x=as.Date(yearmonth), ymin=lower_95, ymax=upper_95), alpha=0.1, fill='orange')+
  ...
  geom_line(data = fc_ts[fc_ts$areaID==plt,], aes(x=as.Date(yearmonth), y=ndvi_pred),
            col='darkblue',lwd=0.9,lty=2)+
  geom_line(data = fc_ts[fc_ts$areaID==plt,], aes(x = as.Date(yearmonth), y = ndvi_int),lwd=1,
            col='black')+
  #geom_line(data=l8_fc_plt_dt[l8_fc_plt_dt$plotID==plt,], 
  #          aes(x=as.Date(yearmonth), y=normalize(prcp, method='range',c(0,0.3))), col='blue')+
  annotate('text', x=as.Date('2021-12-01'),y=0.9, 
           label=paste('MAEp =',round(mae[mae$areaID==plt,]$mae,3)),
           size=3.5)+
  labs(y='NDVI')+
